...
export default function AccionesFinales({ comercial, ofertaId }) {
  ...
  const abrirCalendly = () => {
    trackEvent(ofertaId, 'btn_agendar')
    if (window.Calendly) {
  ...
  const handleAceptar = () => {
    trackEvent(ofertaId, 'oferta_aceptada')
    setEstado('aceptada')
  }

  const handleEnviarRechazo = () => {
    if (!motivoSeleccionado) return
    trackEvent(ofertaId, 'oferta_rechazada', {
      motivo:     motivoSeleccionado,
      comentario: comentarioLibre || null,
    })
    setEstado('rechazada')
  }

  const handleWhatsApp = () => {
    trackEvent(ofertaId, 'btn_whatsapp')
  }

  const handleEmail = () => {
    trackEvent(ofertaId, 'btn_email')
  }
